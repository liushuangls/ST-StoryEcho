...
L1 保留 v0.21.20 的组织规则，不强制增加历史阶段标题或固定分类。2026-09-20 的标题组织候选未通过效果验收，已从生产路径撤回，仅在评测目录保留复现工具和记录；详见[效果测试记录](docs/PROMPT_EVAL_L1_HISTORY_STRUCTURE_EXPERIMENT.md)。已有总结不自动改写。

v0.21.21 优先保留独有重要事实及最短必要因果链，允许信息密集的总结适度变长；日常事件若承载共同经历、承诺或状态变化，不因场景普通而一概省略。各层都要求遵守局部证据边界：未提及不等于从未发生，也不能据此补写“首次”或“来历未知”；新片段未重述旧事实不等于推翻旧事实。这些规则不能保证模型完全无遗漏或误判。

接口明确报告达到输出上限（如 `length` / `MAX_TOKENS`）时，本次不完整总结不会写入；保留未覆盖原文、原有总结和已完成的重建草稿，不推进失败批次的覆盖范围。不会因此自动加预算或切换模型；没有结束原因元数据时，不凭正文长短推断截断。已有历史总结不会被自动删除。
