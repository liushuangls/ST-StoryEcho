...
项目处于 Alpha 测试阶段。目前已有两种 LLM Provider、OpenAI 兼容与火山方舟多模态两种外部 Embedding、按配置满批抽取、3000 Token 受控抽取参考、候选质量门槛、六类事件整理动作、同状态槽多目标失效、承诺稳定逻辑键、LLM 查询改写与规则降级、聊天元数据存储、Vector Storage 增量同步、滑动窗口、生成拦截、运行统计和调试报告。合并记忆会按完整 `sourceHistory` 判断是否越过窗口边界，避免“最新确认仍在窗口内”时漏召回更早的关键状态。
...
剧情抽取同样按“每批抽取轮数”累计完整的“用户 + AI”轮次，默认 5 轮才调用一次；不足一批时不调用抽取 LLM，也不裁剪这段原文。动态召回默认最多 3 条，并会先过滤分数过低或与最佳结果差距明显的尾部候选；“最多召回事件”是上限，不保证每轮填满。

抽取模型收到 StoryEcho 自己的抽取 system prompt、该批真实聊天楼层，以及一个总预算默认 3000 Token 的受控只读参考块。角色部分仅取身份、Persona、描述、性格与场景；世界书部分只从当前聊天实际关联的书中选择被这批旧消息直接命中的最多 5 条。角色卡 system/jailbreak、作者注释、示例对话、欢迎语、主提示词和 Prompt Manager 预设始终不传。参考块只能用于实体和别名消歧，每条记忆必须引用真实聊天中的 `sourceMessageIds`，没有有效来源的候选会被拒绝。主连接后台任务在酒馆提供相应字段时会临时关闭重推理并采用确定性采样；文本补全 API 仍可能沿用 instruct 格式。其他扩展若监听酒馆的 raw prompt 事件并主动改写请求，则仍可能影响后台调用。
