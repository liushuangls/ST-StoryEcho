...
项目处于 Alpha 测试阶段。目前已有两种 LLM Provider、OpenAI 兼容与火山方舟多模态两种外部 Embedding、按配置满批抽取、回复后单批后台整理、3000 Token 受控抽取参考、可见剧情正文清洗、稳定身份资料抽取、候选质量门槛、六类事件整理动作、User事实优先、同状态槽多目标失效、承诺稳定逻辑键、LLM 查询改写与规则降级、可查看/编辑/删除的聊天元数据、Vector Storage 增量同步、滑动窗口、生成拦截、跨阶段当前状态校正、运行统计和调试报告。合并记忆会按完整 `sourceHistory` 判断是否越过窗口边界，避免“最新确认仍在窗口内”时漏召回更早的关键状态。
