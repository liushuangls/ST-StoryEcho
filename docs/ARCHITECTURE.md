...
输出只保留该阶段的时间线、人物关系、目标、关键变化、未解决问题和阶段结束局势，不承担精确实体检索。服务不会把旧总结重新发送给总结 LLM，因此既避免滚动改写造成的信息漂移，也让已生成的提示前缀保持稳定。保存新条目前必须再次计算本批原文哈希；哈希不一致、输出为空或请求失败时不追加条目，也不推进覆盖游标。自动路径每次正常生成最多处理一批；主动处理历史可持续追赶所有完整批次，但不足 `N` 轮的尾段继续保留原文。
...
历史块过滤聊天数组里的 system 消息，并在其前加入独立的 `story_echo_reference_context`。参考上下文默认总预算 3000 Token，角色卡最多优先使用 1200 Token，只读取身份、Persona、描述、性格与场景；其余预算用于当前聊天实际关联世界书中被该历史批次直接命中的最多 5 条。匹配使用待抽取快照而不是当前聊天末尾，且不纳入 constant/强制条目。角色卡 system/jailbreak、作者注释、示例对话、欢迎语、主提示词、阶段总结、动态召回和 Prompt Manager 预设不在输入中。
...
`SUPERSEDE` 将旧记忆标为 `superseded`、记录新旧 ID关系并创建最新有效记忆。应用主决策后，本地还会按规范化状态槽扫描全部候选旧记忆：位置/当前位置/存放地点、持有/保管者、知情范围、承诺状态、真伪状态等近义属性归入同一槽，一次更新可取代多个旧目标。实体必须按完整规范名匹配，因此“白塔药铺”和“北境白塔”不会仅因共享“白塔”而串线。对于旧版本遗留的复合记忆，若只取代其中一个实体，系统还会从不含被改实体的独立分句生成残余记忆，避免另一个事实随整条旧记忆一起丢失；不能安全拆分时宁可不猜。任何检索文本变化都会分配新向量哈希，同时把旧哈希放入 `pendingVectorDeleteHashes`，防止旧事实继续被语义召回。
...
LLM改写成功时，`Retriever` 使用改写后的一句话执行一次 Vector Storage查询，同时仍用原始用户输入和 AI 场景尾部做实体精确匹配。改写失败或用户选择本地模式时，`QueryBuilder` 回退为两个彼此独立的查询：当前用户输入和上一条 AI 回复末尾最多 500 字；普通权重为 `1.0 / 0.35`，弱语义输入改为 `0.25 / 1.0`。

每个向量通道获取最终数量 3 倍的候选。任一路失败不影响其他通道、固定记忆和实体关键词降级召回。
...
重排后保留固定记忆，并过滤综合分低于 2 或低于本轮最佳非固定结果 40% 的尾部候选；随后再应用“最多召回事件”和 Token 预算。默认最多召回 3 条，因此结果数是 0～3，而不是强行填满 3 条。`resolved`表示已完成承诺、已核验线索或已否定传言等当前有效事实，不统一降权；只有`superseded`和`invalid`记忆会在排序前排除。后续若服务端接口开放 score，再替换该部分。
...
每次生成保存一个轻量 `InspectionRecord`，并累计抽取、抽取参考、整理、向量和裁剪指标。调试模式额外在聊天元数据中保留最近 50 条有界轨迹，包括抽取参考字段、命中世界书标识、Token 数、有界预览、合资格记忆 ID、各向量通道的哈希与排名以及最终选中 ID。设置面板和可复制报告展示实际结构化注入文本，而不是仅展示原始 `injectionText`。报告明确排除 API Key和自定义 Base URL，但包含定位问题所需的角色/世界书参考预览、剧情查询、整理动作和召回文本。
...
连续聊天不会把完整历史反复发送给后台模型：阶段总结只读取下一批尚未覆盖的原文，默认每 10轮新增一次且不重写旧条目；抽取只处理尚未索引的新分块，并在窗口外累计满默认 5个完整轮次后才调用一次；检索查询改写仍按配置在每次需要召回时读取最新用户输入和最近 3条非系统消息；正常角色生成只保留未总结尾段、至少 W轮近期原文、最近 S条有界总结和有限条召回记忆。因此 LLM 与 Embedding 总成本随新增剧情线性增长，单次生成输入受 `W + N + S` 约束；等待满批时，未处理原文会临时扩大安全窗口。全部阶段总结元数据会随批次数线性增长，但每 10轮只有一条短文本，且请求只切片最近 S条。首次为已有超长聊天回填仍会产生一次性线性成本。
...
- 为发现任意旧楼层编辑、删楼和分支截断，生成前的已索引前缀校验为 `O(N)`；它只在本地序列化并计算 SHA-256，不调用 LLM或 Embedding。几千楼时成本随聊天文本线性增长，但仍远小于网络模型调用；若真实超长聊天中成为瓶颈，可进一步接入 SillyTavern消息变更事件或分段 Merkle 哈希；
- 没有待写入或待删除向量时，生成前同步直接返回，不读取完整向量哈希列表；设置面板也只在初次打开、手动刷新和处理历史后读取向量总数。
