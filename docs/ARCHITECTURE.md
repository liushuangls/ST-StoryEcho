...
输出只保留该阶段的时间线、人物关系、目标、关键变化、未解决问题和阶段结束局势，不承担精确实体检索。服务不会把旧总结重新发送给总结 LLM，因此既避免滚动改写造成的信息漂移，也让已生成的提示前缀保持稳定。保存新条目前必须再次计算本批原文哈希；哈希不一致、输出为空或请求失败时不追加条目，也不推进覆盖游标。AI 回复写入聊天后，后台队列最多处理一批抽取和一批总结；下一次生成前仍保留同样的单批安全补齐路径。主动处理历史可持续追赶所有完整批次，但不足 `N` 轮的尾段继续保留原文。
...
每个向量通道至少获取 24 条候选，显式询问多个实体时在 Token 预算内先保证每个实体各有一个最佳结果；普通查询仍服从配置上限，多实体“分别/各自/逐一”查询最多可临时扩展到 8 条。任一路失败不影响其他通道、固定记忆和实体关键词降级召回。
...
连续聊天不会把完整历史反复发送给后台模型：阶段总结只读取下一批尚未覆盖的原文，默认每 10轮新增一次且不重写旧条目；抽取只处理尚未索引的新分块，并在窗口外累计满默认 5个完整轮次后才调用一次。AI 回复完成事件会延迟触发后台队列，每次最多各处理一个抽取与总结分块，避免安装在旧长聊天后意外产生无界费用；生成前仍有同样的安全补齐。检索查询改写按配置在每次需要召回时读取最新用户输入和最近 3条经过正文清洗的非系统消息；正常角色生成只保留未总结尾段、至少 W轮近期原文、最近 S条有界总结、最多 600 Token 的变更状态校正和有限条召回记忆。因此 LLM 与 Embedding 总成本随新增剧情线性增长，单次生成输入受 `W + N + S` 约束；等待满批时，未处理原文会临时扩大安全窗口。全部阶段总结元数据会随批次数线性增长，但每 10轮只有一条短文本，且请求只切片最近 S条。首次为已有超长聊天回填只会由用户主动“处理窗口外历史”或后续多轮单批追赶，不会在一次后台事件中自动跑完。
