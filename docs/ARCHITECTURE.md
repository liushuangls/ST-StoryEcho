...
连续聊天不会把完整历史反复发送给后台模型：阶段总结只读取下一批尚未覆盖的原文，默认每 10轮新增一次且不重写旧条目；抽取只处理尚未索引的新分块，并在窗口外累计满默认 5个完整轮次后才调用一次。AI 回复完成事件会延迟触发后台队列，每次最多各处理一个抽取与总结分块并重试待同步向量，避免安装在旧长聊天后意外产生无界费用；生成前不执行这三类写任务。检索查询改写按配置在每次需要召回时读取最新用户输入和最近 3条经过正文清洗的非系统消息；正常角色生成只保留未总结尾段、至少 W轮近期原文、最近 S条有界总结、最多 600 Token 的变更状态校正和有限条召回记忆。因此 LLM 与 Embedding 总成本随新增剧情线性增长，单次生成输入受 `W + N + S` 约束；等待满批或后台落后时，未处理原文会临时扩大安全窗口。全部阶段总结元数据会随批次数线性增长，但每 10轮只有一条短文本，且请求只切片最近 S条。首次为已有超长聊天回填只会由用户主动“处理窗口外历史”或后续多轮单批追赶，不会在一次后台事件中自动跑完。
