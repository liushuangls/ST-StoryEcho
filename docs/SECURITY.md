# StoryEcho 安全边界

## 凭据

StoryEcho 只有自定义 LLM 一类可选凭据。Base URL、模型和 API Key 保存在当前 SillyTavern 用户的 `extensionSettings.story_echo`，用于刷新恢复和多端同步。

这不是 Secret Manager：同页面扩展以及能读取酒馆用户设置的人可能看到明文 Key。建议使用限额、限权、可撤销的独立 Key。

Key 不写入：

- 聊天消息；
- `chatMetadata.story_echo`；
- 任何层级的总结；
- 控制台日志；
- Token 卡片；
- 诊断报告。

诊断报告会同时脱敏配置的 API Key 和自定义 Base URL，包括调试错误详情中的重复值。

## 网络

- 主连接通过 SillyTavern 的 `generateRaw` 或兼容的同源流式入口；
- 自定义 OpenAI 兼容连接通过同源 `/api/backends/chat-completions/generate`；
- 浏览器不会直接向第三方域名发送 Key；
- 默认只允许 HTTPS；
- HTTP 必须由用户显式开启，且只应连接可信内网；
- Base URL 经过协议、用户信息、片段和路径规范化；
- API Key 拒绝换行和异常长度；
- 单次 L1 或高层总结请求截止时间为 5 分钟，Provider 另有 10 分钟防御性硬上限；
- 响应使用 2 MiB 流式上限，错误详情限制为 500 字符并脱敏；
- 本地超时和可识别的上游 408/504/524 会进入一次有界重试。

## 提示词与内容

- 世界书内容的 `<`、`>` 会转为全角字符，避免伪造 StoryEcho 协议标签；
- 世界书明确标记为背景设定，不作为事件发生证据；
- 分层总结明确标记为历史数据，不是待执行指令；
- 当前用户输入、近期原文及时间更近的低层总结优先于较早高层总结；
- 内部 LLM 请求携带随机 nonce，防止被生成拦截器递归处理；
- UI 使用 `textContent` 或表单 value 显示动态内容，不把聊天文本拼接为可执行 HTML；
- 确认弹窗会转义 HTML 特殊字符。

## 扩展间读取

只读公共 API 会向同一页面中的其他扩展提供当前总结前沿及最近一次 StoryEcho 注入文本。返回值不包含自定义 LLM Key、Base URL、内部请求正文或未进入总结的聊天原文，并且全部是冻结副本。SillyTavern 页面扩展本身通常已经可以访问聊天及 `chatMetadata`；用户仍应只安装可信扩展。

该 API 不提供写入、删除、重新生成或修改覆盖游标的方法。最近注入只保存在当前页面内存中，切换聊天或停用 StoryEcho 后清除，不额外持久化剧情正文。

## 一致性与删除

L1 和高层压缩在提交前都会复核聊天所有权、原文来源和总结前沿。任何一项变化都会丢弃未提交结果；取消和失败不会覆盖原总结。

设置页可以删除总结，但不会删除聊天原文。删除最新总结会让对应原文重新参与处理；删除旧总结保留覆盖墓碑。清空诊断只清除指标、检查记录和调试轨迹，不影响分层总结。

旧版状态升级为 Schema 3 时，仅迁移可验证的阶段总结为 L1；旧全局骨架及未被当前模型声明的字段会被丢弃。
